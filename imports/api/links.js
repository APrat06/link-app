...
if (Meteor.isServer){Meteor.publish('links', function(){this.userId; return Links.find({userId:this.userId})})}
...
Meteor.methods({
    ...
    'links.setVisibility'(_id, visible){
        if(!this.userId){throw new Meteor.Error("not authorized")};
        new SimpleSchema(
            {_id: {type: String, min: 1}, visible: {type: Boolean}}
            ).validate({_id, visible});
        Links.update({_id, userId: this.userId}, {$set: {visible}})
    },
    'links.trackVisit'(_id){
        new SimpleSchema(
            {_id: {type: String, min: 1}}).validate({_id});
